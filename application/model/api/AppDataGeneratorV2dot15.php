...
class Model_Api_AppDataGeneratorV2dot15
{
    public static function generujJson($postData)
    {
        ...
        if($user !== null) {
            ...
            $res['status'] = 'ERROR';
            $res['errorMessage'] = 'Nieprawidłowy login, hasło lub użytkownik nieaktywny.';
        }
        ...
        if($res['status'] == 'OK') {
            $res['promotions'] = self::getPromotionsData();
    ...
    private static function getPromotionsData()
    {
        $res = array();
        ...
        // TODO: ograniczyc dla zalogowanego przedstawiciela
        //$mapper->filterBy('przedstawicielId', 1);
        $orders = $mapper->find();
        $res['orders'] = array();
