...
class Model_Api_AppDataGeneratorV2dot28
{
    public static function generujJson($postData)
    {
                    ...
                    foreach ($ordersData as $orderData) {
                        if (isset($orderData['remoteId'])) {
                            ...
                            if ($order !== null) {
                                ...
                                } elseif ($orderData['statusId'] == Model_Promocje_OrderStatusMapper::STATUS_NOWE) {
                                    $order->statusId = Model_Promocje_OrderStatusMapper::STATUS_CZEKA_NA_ZATWIERDZENIE;
                                    $supervisor = $user->getSupervisor();
                                    if ($supervisor->id > 0) {
                                        $order->nextEditorId = $supervisor->id;
        ...
        $res['asortymentSieci'] = array();//self::getAsortymentSieci();
        ...
        foreach ($res as $key => $val) {
            if ($key == 'asortymentSieci') {
                ...
                $jsonParts[$key] = self::getAsortymentSieciJson();
            } else {
        ...
        foreach ($klienci as $klient) {
            ...
            foreach ($values as $value) {
                ...
                $produkty[] = array(
                    ...
                    'kategoria' => $asortymentProdukt->kategoria,
                    'segment' => $asortymentProdukt->segment
                );
            }
            $asortymentSieci[] = array(
