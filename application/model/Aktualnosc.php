...
class Model_Aktualnosc
{
	...
	public $foto_dir = 'aktualnosci';
	public $modul_adres = 'aktualnosci';
	public $table_prefix = 'aktualnosc';
	public $main_table = "aktualnosci";
	public $opis_table = "aktualnosci_opisy";
	public $podstrony_table = "aktualnosci_podstrony";
	public $podstrony_table_prefix = "podstrona";
	public $foto_table = "aktualnosci_foto";
	...
	//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
	public function podajSkroconaOpis($jezyk_id, $dlugosc=100)
	{

		$text_tmp = strip_tags($this->tresc[$jezyk_id]);
	...
	public function zapisz()
	{
		...
		if((int)$this->id > 0) {
			...
			foreach($a_jezyki as $idJezyka => $skrotJezyka) {
				...
				$queryCount = $db->query('SELECT * FROM '.$this->opis_table.' WHERE aktualnosc_id = '.(int)$this->id.' AND jezyk_id = '.(int)$idJezyka.'');
	...
	//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
	public function zapiszZdjecie($aktualnosc_id, $plik_upload, $token='')
	{
		$db = Core_DB::instancja();
		$prefix = time().'_';
		//====================================
		...
		$sql_glowne = "SELECT count(*) AS ile FROM  ".$this->foto_table." WHERE ".$this->foto_table_prefix."_aktualnosc_id=".(int)$aktualnosc_id." AND ".$this->foto_table_prefix."_token=".(int)$token;
		...
		//====================================
		$sql = "INSERT INTO ".$this->foto_table." ( ".$this->foto_table_prefix."_aktualnosc_id, ".$this->foto_table_prefix."_sciezka, ".$this->foto_table_prefix."_glowna, ".$this->foto_table_prefix."_token,".$this->foto_table_prefix."_kolejnosc ) VALUES (".(int)$aktualnosc_id.",'".$plik_nazwa."',".$glowne.",'".(int)$token."',9999)";		
