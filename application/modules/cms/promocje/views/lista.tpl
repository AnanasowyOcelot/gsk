...
<tr>
	...
	<th><a href="{$link}index/{$linkParams}col:kod_icoguar">Kod Icoguar</a></th>
	<th><a href="{$link}index/{$linkParams}col:data">Miesiąc / Rok</a></th>
</tr>
{foreach from=$lista  item=rekord}
	...
	{/if}
	{if $uprawnienia.edytowanie==1}
		<td class="colEdit"><a class="buttonEdit" href="{$link}edytuj/{$linkParams}{$primaryKeyName}:{$rekord->id}">&nbsp;</a></td>
	{/if}
	{if $uprawnienia.usuwanie==1}
	...
	<td><a class="linkEdit" href="{$link}edytuj/{$linkParams}{$primaryKeyName}:{$rekord->id}">{$rekord->kod_icoguar}</a></td>
        {assign var='time' value=$rekord->data|strtotime}
	<td><a class="linkEdit" href="{$link}edytuj/{$linkParams}{$primaryKeyName}:{$rekord->id}">{'m'|date:$time} / {'Y'|date:$time}</a></td>
    </tr>
{/foreach}
</table>
