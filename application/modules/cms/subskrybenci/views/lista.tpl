...
<div class="listaHeader">
    <table><tr>
        ...
        <td class="middle">
            ...
            <a href="{$link}csv" style="text-align:right;">generuj plik CSV</a>
        </td>
        <td class="right">&nbsp;</td>
    </tr></table>
</div>
...
<tr>
	...
	<th><a href="{$link}index/{$linkParams}col:email">Email</a></th>
</tr>
{foreach from=$lista  item=rekord}
	...
	{/if}
	{if $uprawnienia.edytowanie==1}
		<td class="colEdit"><a class="buttonEdit" href="{$link}edytuj/{$linkParams}id:{$rekord->id}">&nbsp;</a></td>
	{/if}
	{if $uprawnienia.usuwanie==1}
		<td class="colDelete"><a class="buttonDelete" href="{$link}usun/{$linkParams}id:{$rekord->id}">&nbsp;</a></td>
	{/if}
	<td  class="colID">{$rekord->id}</td>
    <td><a class="linkEdit" href="{$link}edytuj/{$linkParams}id:{$rekord->id}">{$rekord->email}</a></td>
    </tr>
{/foreach}
</table>
<div class="listaFooter">
	...
	{/if}
            <div class="listaPagination">{$porcjowarka}</div>
        </td>
        <td class="right">&nbsp;</td>
    </tr></table>
</div>
