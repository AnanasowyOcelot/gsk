...
<tr>
	...
	<th><a href="{$link}index/{$linkParams}col:nazwa">Pierwsza strona</a></th>
...
{foreach from=$lista  item=rekord}
    ...
    <td><img src="{$rekord->getFirstPageImagePath()}" style="max-height: 150px; max-width: 200px;" /></td>
    <td><a class="linkEdit" href="{$link}edytuj/{$linkParams}{$primaryKeyName}:{$rekord->id}">{$rekord->nazwa}</a></td>
    <td><a class="linkEdit" href="{$link}edytuj/{$linkParams}{$primaryKeyName}:{$rekord->id}">{if $rekord->active}tak{/if}</a></td>
    </tr>
{/foreach}
</table>
